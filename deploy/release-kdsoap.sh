...
PROJECT=kdsoap
FORMAL_PROJECT=KDSoap #also used for the CMake options
...
mkdir -p "$PROJECT-$release/kdwsdl2cpp"
#copy in the submodules by-hand as git archive won't deal with submodules
git submodule update --recursive --init
cp -r kdwsdl2cpp/libkode "$PROJECT-$release/kdwsdl2cpp"
#copy in the generated API documentation
mkdir -p "$PROJECT-$release/docs"
cp -r "$TOP/build-docs/docs/api/html" "$PROJECT-$release/docs"
mv "$PROJECT-$release/docs/html" "$PROJECT-$release/docs/refman"
cp "$TOP/build-docs/docs/api/$PROJECT.tags" "$PROJECT-$release"
#tar up everything else
git archive --format=tar --prefix="$PROJECT-$release/" "$PROJECT-$X.$Y" >"$PROJECT-$release.tar"
