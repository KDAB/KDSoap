...
# Client code (with generated type classes); also used by servers
...
kdwsdl_h.input = KDWSDL KDWSDL_SERVER
...
kdwsdl_impl.input = KDWSDL KDWSDL_SERVER
...
# Server-only processRequest code (KDWSDL_SERVER)

kdwsdl_server.commands = $$KDWSDL2CPP -server $${KD_MOD_SWSDL}${QMAKE_FILE_BASE}$${first(QMAKE_EXT_H)} ${QMAKE_FILE_IN} -o ${QMAKE_FILE_OUT}
#kdwsdl_server.depend_command = "$$KDWSDL2CPP" -d "${QMAKE_FILE_IN}"
kdwsdl_server.output = $$WSDL_SOURCES_DIR/$${KD_MOD_SWSDL}${QMAKE_FILE_BASE}$${first(QMAKE_EXT_CPP)}
kdwsdl_server.depends = $$KDWSDL2CPP
kdwsdl_server.input = KDWSDL_SERVER
kdwsdl_server.variable_out = WHATEVER_SOURCES
kdwsdl_server.dependency_type = TYPE_C
kdwsdl_server.CONFIG += target_predeps
kdwsdl_server.name = KDWsdl2Cpp SOURCE ${QMAKE_FILE_IN}
silent:kdwsdl_server.commands = @echo kdwsdl2cpp -server ${QMAKE_FILE_IN} && $$kdwsdl_server.commands
QMAKE_EXTRA_COMPILERS += kdwsdl_server
